...
if selected_rooms:
    ...
    for idx, row in unit_df.iterrows():
        ...
        # ใช้ราคาที่เสนอ หรือ bottom ถ้ายังไม่กรอก
        project_value = user_price if user_price > 0 else bottom

        gp_unit = ((project_value - cost) / project_value * 100).round(2)
        gp_diff = (gp_unit - gp_committed).round(2)

        all_prices.append(user_price)
        gp_values.append(gp_unit)
        cost_total += cost
        ...
        <div style='border: 1px solid #ddd; background-color: #fefefe; padding: 1rem; border-radius: 10px; margin-bottom: 1rem;'>
            <strong>ยูนิต:</strong> {room_no}<br>
            <strong>Asking Price:</strong> {asking:,.2f} บาท<br>
            <strong>Bottom Price:</strong> {bottom:,.2f} บาท<br>
            <strong>ราคาที่เสนอ:</strong> {user_price:,.2f} บาท<br><br>

            <strong>GP (รายแปลง):</strong> {gp_unit:.2f}%<br>
            <strong>GP (Committed):</strong> {gp_committed:.2f}%<br>
            <strong>ผลต่าง GP:</strong> <span style='color: {"green" if gp_diff >= 0 else "red"}'>{gp_diff:+.2f}%</span>
        </div>
        """, unsafe_allow_html=True)

    # GP รวมของโครงการ
    gp_project = ((price_total - cost_total) / price_total * 100).round(2) if price_total > 0 else 0
    gp_project_diff = (gp_project - gp_committed).round(2)
